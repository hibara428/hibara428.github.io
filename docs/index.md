...
| 好きな運用管理ツール | 探しています(しばらくはTerraform推しでした) |
...
フロントエンドからのHTTPリクエストに対してコンテンツデータを配信するRESTful APIを設計・実装。APIは認証・認可の機能を持つ。サービスごとに複数バージョンが存在。
...
新規API設計時には、既存インタフェースに対して以下の改善に注力。
...
構成を変更しつつ、既存開発フローに対応できるように考慮。また、営業部門が容易かつ安全に特定顧客専用サンプル公開環境を用意できるPHPアプリケーションを実装。認証にはCognitoを利用。
...
新規ハイブリッドクラウド基盤構築プロジェクトのプロジェクト管理補佐として、上流のプロジェクト管理に従事。
...
- データセンタにある100台以上の物理サーバ、およびVMWare基盤
- 10名前後のチーム
